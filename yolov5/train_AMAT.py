...
#%% AMAT for regression problem part



def AMAT_update_margin(E, delta, max_margin, flag1, margin_new):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, idx) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, advc, Xn = AMAT_loss(model, imgs, targets.to(device),
                                                        norm_type= norm_type,
        ...
        # end epoch ----------------------------------------------------------------------------------------------------
        AMAT_update_margin(E, delta, noise, flag1,  E_new)
        # draw margins

        plt.hist(E.cpu().numpy(), bins=100)
...
def parse_opt(delta , known=False):
    ...
    parser.add_argument('--project', default=ROOT / 'runs/AMAT_grid', help='save to project/name')
    parser.add_argument('--name', default='BCCD_AMAT_Grid_L2_d'+str(delta), help='save to project/name')
    ...
    parser.add_argument('--delta', default=delta, help='delta in amat')
...
"""
def run(delta, **kwargs):
    # Usage: import train; train.run(data='coco128.yaml', imgsz=320, weights='yolov5m.pt')
    opt = parse_opt(delta, True)
