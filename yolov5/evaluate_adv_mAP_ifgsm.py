...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if noise>0:
            ...
            im = pgd_attack(model.model, im, targets, noise_norm=noise, 
                            norm_type=2, max_iter = max_iter, 
                            step = step,loss_fn = compute_loss, rand_init=False)
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("mAPresult_IFGSM10.pdf",bbox_inches='tight')
    plt.clf()
        
    
    import csv
    fields1 = ["noise"]+[str(i) for i in noises]+["AUC"]
    with open("mAPresult_IFGSM10.csv",'w') as csvfile:
