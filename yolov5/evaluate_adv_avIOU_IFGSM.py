...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if noise>0:
            ...
            im = pgd_attack(model.model, im, targets, 
                            noise_norm=noise, norm_type=2, 
                            max_iter = max_iter, step = step,
                            loss_fn = compute_loss, rand_init=False)
...
if __name__ == "__main__":
    models = ['runs/train/BCCD/weights/last.pt',
              ...
              'runs/AMAT_grid/BCCD_AMAT_Grid_L2_d3.5/weights/last.pt']
    
    for p in models:
        assert(os.path.exists(p))
    print('all the models exist!!!!!!!!!!!!!!!!!!!!!')
    model_names = ['STD', 'SAT1','SAT10','SAT15','AMAT']
    ...
    for i, n in enumerate(model_names):
        plt.plot(noises, this_list[i][1:5], color = colors[i], label = model_names[i]+"AUC Robust"+str(this_list[i][-1]))
    plt.legend()
    plt.grid(True)
    plt.savefig("avIOU result IFGSM.pdf",bbox_inches='tight')
    plt.clf()
        
    import csv
    fields1 = ["noise"]+[str(i) for i in noises]+["AUC Robust"]
    with open("avIOU result IFGSM.csv",'w') as csvfile:
