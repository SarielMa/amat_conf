...
if __name__ == "__main__":
    deltasList = [i/10 for i in range(5, 55,5)]
    #deltasList = [0.5]
    models = ['runs/AMAT_grid/BCCD_AMAT_Grid_L2_d'+str(d)+'/weights/last.pt' for d in deltasList]
    
    for p in models:
        assert(os.path.exists(p))
    print('all the models exist!!!!!!!!!!!!!!!!!!!!!')
    model_names = ["Delta"+str(d) for d in deltasList] 
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("avIOU result 100PGD.pdf",bbox_inches='tight')
    plt.clf()
        
    import csv
    fields1 = ["noise"]+[str(i) for i in noises]+["AUC Robust"]
    with open("avIOU result 100PGD.csv",'w') as csvfile:
