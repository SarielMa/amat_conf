...
class Tester(object):
    ...
    def validate_ifgsm(self, net, loss_fn, noise=0, norm_type = 2, max_iter = 10):
        ...
        for img, mask, guassian_mask, offset_y, offset_x, landmark_list in tqdm(dataloader_val):
            ...
            if noise > 0:
                step = 5*noise/max_iter
                img = pgd_attack(net, img, mask, offset_y, offset_x, guassian_mask, noise, norm_type, max_iter, step, \
                                  loss_fn = loss_fn, rand_init = False)
            with torch.no_grad():    
                heatmap, regression_y, regression_x = net(img)               
                pred_landmark = voting(heatmap, regression_y, regression_x, Radius)   
                evaluater.record(pred_landmark, landmark_list)    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", default='test_IFGSM', help="position of the output dir")
    ...
    folders = ["SAT5"]
    folders = ["SAT1","SAT3","SAT5", "AMAT", "STDBCE", "STDDICE"]
    #folders = ["PGD_1_500"]
    ...
    for n, loss_fn in enumerate(losses):
        ...
        for i, folder in enumerate(folders):
            ...
            for noise in noises:
                ...
                MRE= tester.validate_ifgsm(net, noise = noise, loss_fn = loss_fn)
                print("Testing MRE {}".format(MRE))
                MRE_list.append(MRE)

            #plt.yscale("log")                           
            plt.plot(noises,MRE_list, color = cm(1.0*i/len(folders)), label =folder+" AVG. "+str(round(np.mean(MRE_list))) )                                     
            plt.ylabel("MRE (mm)")
            plt.xlabel("noise (L2)")
