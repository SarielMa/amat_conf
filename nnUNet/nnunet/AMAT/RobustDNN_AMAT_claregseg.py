...
from nnunet.AMAT.RobustDNN_PGD import get_noise_init, normalize_grad_, clip_norm_
#%% classification, see more advanced loss functions in RobustDNN_AMAT
...
def repeated_pgd_attack(task,model, X, Y, noise_norm, norm_type, max_iter, step,
    ...
    return Xn, advc
#%%
def AMAT_loss(task, model, X, Y, margin, norm_type, max_iter, step,
             rand_init_norm=None, rand_init_Xn=None,
    ...
    if enable_loss3 == True:
        ...
        # to be consistant with the output of AMAT_loss in RobustDNN_AMAT
        Xn=Xn[idx_n] 
        #Ypn=Ypn[idx_n]
        if idx_n.size(0)>0:   
...
#%%
def AMAT_check_margin_one_batch(model, X, Y, margin, norm_type, max_iter, step,
                               rand_init_norm=None,
                               clip_X_min=0, clip_X_max=1,
                               refine_Xn_max_iter=10,
                               Xn1_equal_X=False,
...
def AMAT_check_margin(model, device, dataloader,
    ...
    for batch_idx, (X, Y, Idx) in enumerate(dataloader):
        ...
        flag1_, flag2_, margin_ = AMAT_check_margin_one_batch(model, X, Y, margin_, norm_type, max_iter, step_,
    ...
    return flag1, flag2, margin_new
#%%
def AMAT_update_margin_OLD(margin, delta, max_margin, flag1, flag2, margin_new):
    # margin: to be updated
    # delta: margin expansion step size
    # max_margin: maximum margin
    # flag1, flag2, margin_new: from AMAT_check_margin
...
def AMAT_update_margin(args, delta, max_margin, flag1, flag2, margin_new):
    # margin: to be updated
    # delta: margin expansion step size
    # max_margin: maximum margin
    # flag1, flag2, margin_new: from AMAT_check_margin
...
#%%
def AMAT_estimate_margin(model, device, dataloader,
    ...
    for n in range(1, len(margin_level)):
        ...
        flag1, flag2, Enew =AMAT_check_margin(model, device, dataloader,
