...
from nnunet.AMAT.RobustDNN_AMAT_claregseg import AMAT_update_margin
import os.path
from nnunet.utilities.nd_softmax import softmax_helper
...
class NetworkTrainer(object):
    ...
    class PGD_params_D2:
        def __init__(self):           
            #used to pass parameters to AMAT iteration
            self.noise = 20
            self.norm_type = 2
            self.max_iter = 20
    ...
    def run_AMAT_training(self, counter):
        if not torch.cuda.is_available():
            self.print_to_log_file("WARNING!!! You are attempting to run training on a CPU (torch.cuda.is_available() is False). This can be VERY slow!")
        ...
        while self.epoch < self.max_num_epochs:
            ...
            self.network.train()
            for c in range(self.num_batches_per_epoch):
                l, flag1, flag2, E_new = self.run_AMAT_iteration(self.tr_gen, args,flag1, flag2, E_new, True)
            ...
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])
            #------update the margin
            AMAT_update_margin(args, args.delta, args.noise, flag1, flag2, E_new)
            print('AMAT_update_margin: done, margin updated')    
        ...
        # now we can delete latest as it will be identical with final
        if isfile(join(self.output_folder, "model_AMAT_latest.model")):
            os.remove(join(self.output_folder, "model_AMAT_latest.model"))
        if isfile(join(self.output_folder, "model_AMAT_latest.model.pkl")):
            os.remove(join(self.output_folder, "model_AMAT_latest.model.pkl"))
