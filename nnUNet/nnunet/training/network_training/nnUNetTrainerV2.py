...
from nnunet.AMAT.RobustDNN_AMAT_claregseg import AMAT_loss
...
from nnunet.AMAT.PGD import pgd_attack
...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def run_AMAT_iteration(self, data_generator, args,flag1, flag2, E_new, do_backprop=True, run_online_evaluation=False):
        """
        gradient clipping improves training stability
        ...
        loss, loss1, loss2, loss3, Yp, advc, Xn, Ypn, idx_n = AMAT_loss(args, self.network, data, Y=target,#Y is a tuple
    ...
    def run_AMAT_training(self):
        ...
        ds = self.network.do_ds
        self.network.do_ds = True
        ret = super().run_AMAT_training(self.dl_tr.counter)#pass the number of samples in training set
        self.network.do_ds = ds
        return ret
 
    def run_AMAT_training_grid(self, params):
        """
        if we run with -c then we need to set the correct lr for the first epoch, otherwise it will run the first
        continued epoch with self.initial_lr
        ...
        ds = self.network.do_ds
        self.network.do_ds = True
        ret = super().run_AMAT_training_grid(self.dl_tr.counter, params)#pass the number of samples in training set
        self.network.do_ds = ds
        return ret
